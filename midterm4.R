...
hotelCal.split <- unlist(strsplit(tolower(gsub("  ", " ", gsub("[[:punct:]]", "", hotelCal))), split=" "))
...
updateDate <- function(dates, old.yr) {
  olddates = dates[grep(old.yr, dates)]
  return(gsub(old.yr, '2015', olddates))
}
...
abbreviate <- function(vector, k){
  unlist(lapply(vector, function(x) paste(unlist(strsplit(x, split=""))[1:k], sep="", collapse="")))
}
