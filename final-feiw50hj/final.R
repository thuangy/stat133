...
name <- "Tina Huang"
github.name <- "thuangy"
email.address <- "thuang@berkeley.edu"
...
cw3 <- mean(ChickWeight[ChickWeight$Time == 0, c("weight")])
  

# [1 pt]
# Create [cw4] a random subset of 200 ChickWeight observations (any 200)
cw4 <- ChickWeight[sample(1:length(ChickWeight$weight), 200), ]
...
# 
onlycollege = infants[infants$ed == "College", ]
t2 <- table(onlycollege$smoke, onlycollege$number)
...
#  
bdiff <- mean(infants[infants$age >= 35, c("bwt")], na.rm=TRUE) - mean(infants[infants$age < 35, c("bwt")], na.rm=TRUE)
...
abline(lm(mtcars$qsec ~ mtcars$hp)$coef)
...
plot(mtcars$wt, mtcars$disp, xlab="Weight of the Car (lb/1000)", ylab="Engine Displacement (cu. in.)", 
     main="Engine Displacement VS Weight", col=colors)
...
plot(WorldBank$fertility.rate, WorldBank$life.expectancy, pch = '.', col=as.numeric(WorldBank$region), xlab="Fertility Rate", ylab="Life Expectancy",
...
par(mfrow=c(1, 2))
plot(WorldBank[WorldBank$year == 1960, c("fertility.rate")], WorldBank[WorldBank$year == 1960, c("life.expectancy")],
     pch = '.', col=as.numeric(WorldBank[WorldBank$year == 1960, c("region")]),
     xlab="Fertility Rate", ylab="Life Expectancy", main = "Life Expectancy VS Fertility Rate in 1960")
abline(v=2.1)
plot(WorldBank[WorldBank$year == 2013, c("fertility.rate")], WorldBank[WorldBank$year == 2013, c("life.expectancy")],
     pch = '.', col=as.numeric(WorldBank[WorldBank$year == 2013, c("region")]),
     xlab="Fertility Rate", ylab="Life Expectancy", main = "Life Expectancy VS Fertility Rate in 1960")
abline(v=2.1)
...
par(mfrow=c(1, 1))
hist(WorldBank[WorldBank$lending == "IDA", c("GDP.per.capita.Current.USD")], breaks=24, xlab="GDP Per Capita (USD)", main="GDP for Countries Where Lending Rate is IDA")
...
par(mfrow=c(2,3))
sapply(rain, function(x) hist(x, main="Rainfall"))
...
standardizeVar <- function(m, cols=TRUE){
  ...
  return(matrix(mat, nrow = nrow(m), ncol=ncol(m), byrow=cols))
}
...
PermDiff <- function(cases, controls, k=5000){
  ...
  permutations = c()
  for(i in 1:k){
    indices = sample(length(total), length(cases))
    cases = total[indices]
    controls = total[indices[table(indices) == 1]]
    permutations[i] = mean(cases) - mean(controls)
  }
  return(permutations)
}
...
NumJackpot <- function(k, B){
  n.jackpot = c()
  for(i in 1:B){
    jackpotnum = sample(1:19, 3)
    jackpotwinners = 0
    for(n in 1:k){
      ticket = sample(1:19, 3)
      if(all(ticket == jackpotnum)){
        jackpotwinners = jackpotwinners + 1
      }
    }
    n.jackpot[i] = jackpotwinners
  }
  return(n.jackpot)
}
...
text3 <- phrases[grep("tata", phrases)]
...
for(i in 1:length(dna)){
  if(length(unlist(strsplit(dna[i], split=""))) %% 3 == 0){
    dna2 = c(dna2, dna[i])
  }
}
