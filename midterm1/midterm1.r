...
subset.threebronze <- SO2012Ctry[(SO2012Ctry$Bronze == 3) | (SO2012Ctry$Bronze > 3), ]
...
medpopByGDPPP <- function(GDPPP.cutoff, GDPPP, pop){
  countries_below_cutoff = c()
  for(i in 1:length(GDPPP)){
    if(GDPPP[i] < GDPPP.cutoff){
      countries_below_cutoff = c(countries_below_cutoff, pop[i])
    }
  }
  return(median(countries_below_cutoff))
}
