...
hisToTheir = function(chvec){
  ...
  theirchvec = gsub("^him$", "them", theirchvec)
...
mostCommonLetter = function(chvec){
  letter = tolower(chvec)
  letter = gsub("[[:digit:]]", "", letter)
  letter = gsub("[[:punct:]]", "", letter)
  letter = unlist(strsplit(letter, ""))
  letter = names(table(letter))[table(letter) == max(table(letter))]
  return(letter)
}
